...
var paper = "";
var Grid = new Array();
var PegArr = new Array(1, 5, 9, 13, 2, 6, 10, 14, 3, 7, 11, 15, 4, 8, 12, 16);
var Turn = true;
var Player = new Array();
var MoveCount = 0;
var RedCircles = new Array();
var PegNums = new Array();
var WhosTurn = "";
...
function DropDown(Peg, PlayerTurn, Lvl)
{
  if (PlayerTurn){
    Color = '#DCBE96';
  } else{
  ...
  Stop = ((4 - Lvl) * 50) + 20;
  Start = GetStart(Peg);
  Beads(Start[0], Start[1], PlayerTurn)
    .animate({"cx": Start[0], "cy": (Start[1] + Stop)}, (1250 - (250 * Lvl)));
}
...
function CheckIt(){
  ...
  if (Winner[0] != "")
  {
    if (Winner[0] == "W")
	{
      ...
      WhosTurn.attr({text: "Winner: White"});
    } 
	else if (Winner[0] == "B")
	{
      Player[1][1]++;
      Player[1][0].attr({text: "" + Player[1][1]});
      WhosTurn.attr({text: "Winner: Black"});
    } 
	else if (Winner[0] == "K")
	{
      Player[2][1]++;
...
function ShowWin(Winner){
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    Start = GetStart(Peg);
	Peg += increment;
    RedCircles[i] = paper.add([{type: "circle",cx: Start[0],
	  cy: (Start[1]+((4-Lvl)*50)+19),r: 16,stroke: "#FF0000","stroke-width":3}]);
  }
